...
export default function MyDistrict() {
  ...
  useLayoutEffect(() => {
    axios
      .get(
        `https://www.googleapis.com/civicinfo/v2/representatives?address=${zipcode}&levels=country&roles=legislatorLowerBody&key=${civic_api_key}`
      )
      .then((res) => {
        fetchForHouse().then((res2) =>
          setRepresentative(
            res2.data.results[0].members.filter(
              (member) =>
                member.state === "NY" &&
                member.ocd_id == res.data.offices[0].divisionId
            )
          )
        );
      });
        ...
        <Box sx={{ minWidth: 200 }}>
          <Card variant="outlined">
            <>
              ...
              <CardActions>
                <Link
                  to={`/representatives/${elem.id}`}
                  state={{ representative: elem }}
                >
